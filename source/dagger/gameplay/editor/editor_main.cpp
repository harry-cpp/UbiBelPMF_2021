...
ECommonSaveArchetype EditorTestGame::Save(Entity entity_, JSON::json& saveTo_)
{
    ...
    if (registry.has<BeanComponent>(entity_))
    {
        saveTo_["bean_component"] = SerializeComponent<BeanComponent>(registry.get<BeanComponent>(entity_));
        archetype = archetype | ECommonSaveArchetype::Beans;
    }
...
void EditorTestGame::Load(ECommonSaveArchetype archetype_, Entity entity_, JSON::json& loadFrom_)
{
    ...
    if (IS_ARCHETYPE_SET(archetype_, ECommonSaveArchetype::Beans))
        DeserializeComponent<BeanComponent>(loadFrom_["bean_component"], registry.emplace<BeanComponent>(entity_));
}
...
struct BeanComponent {
    float howManyBeans = 0.f;
};
    ...
    if (m_IsInEditor)
    {
        ...
        if (selectedItem > 0)
        {
           ...
           /* struct BeanComponent {
            ...
            if (reg.has<BeanComponent>(m_Selected.entity) && ImGui::CollapsingHeader("BeanComponent"))
            {
                BeanComponent& compBean = reg.get<BeanComponent>(m_Selected.entity);
                    float beans[] { compBean.howManyBeans, compBean.howManyBeans };
                    ImGui::DragFloat2("Pivot", beans, 1, -0.5f, 0.5f, "%f", 1);
                    compBean.howManyBeans = beans[0];
            }
            else if (!reg.has<BeanComponent>(m_Selected.entity))
            {
                if (ImGui::Button("Attach Beans"))
                {
                    reg.emplace<BeanComponent>(m_Selected.entity);
                }
            }
